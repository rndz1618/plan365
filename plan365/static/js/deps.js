...
window.Plan365Deps = {
    ...
    async openDetail(task) {
      ...
      this.detailForm = {
        ...
        baseline_due: task.baseline_due || ''
      };
    ...
    async saveDetail() {
      ...
      try {
        ...
        body.is_milestone = !!body.is_milestone;
        body.cascade_schedule = this.cascadeSchedule !== false;
        // do not send baseline fields as writable plan fields
        delete body.baseline_start;
        delete body.baseline_due;
        if (body.is_milestone) {
          const d = body.due_date || body.start_date;
          body.start_date = body.due_date = d || body.start_date;
        }
        ...
        const shifted = (updated.schedule_shifted || []).length;
        this.detailTask = null;
        this.showToast(shifted ? `Task saved · ${shifted} successor digeser` : 'Task saved');
        if (shifted) await this.loadTasks();
        if (this.subView === 'kanban') this.$nextTick(() => this.initKanban && this.initKanban());
        if (this.subView === 'gantt') this.scheduleRenderGantt?.(0);
