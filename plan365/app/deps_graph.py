...
def _dep_adjacency(c, extra_edge=None):
    """Build successor adjacency: predecessor_id -> [successor_ids]."""
    adj = {}
...
def graph_has_cycle(adj):
    ...
    WHITE=0 unvisited, GRAY=1 on stack (back-edge = cycle), BLACK=2 done.
    Returns (has_cycle: bool, cycle_path: list[int] | None).
    """
...
def would_create_cycle(c, predecessor_id, successor_id):
    """True if adding edge predecessor -> successor introduces a directed cycle."""
...
def dependency_path_exists(c, start_id, target_id):
    """True if directed path start -> ... -> target exists (existing edges only)."""
